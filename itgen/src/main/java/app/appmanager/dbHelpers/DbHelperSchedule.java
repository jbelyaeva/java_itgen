...
public class DbHelperSchedule {
  ...
  public ScheduleData lastStudent() {
    Datastore datastore = morphiaSessionFactoryUtil();
    Query<ScheduleData> q = datastore.createQuery(ScheduleData.class);
    long count = q.count();
    List<ScheduleData> student = q.find().toList();
    ScheduleData lastSchedule = student.get(Math.toIntExact(count - 1));
    return lastSchedule;
  }
}
