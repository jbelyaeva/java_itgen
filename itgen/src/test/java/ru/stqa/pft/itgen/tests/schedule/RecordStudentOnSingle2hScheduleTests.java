...
public class RecordStudentOnSingle2hScheduleTests extends TestBase {
  ...
  @Test
  public void testRecordStudentOnSingle2h() {
    app.goTo().menuSchedule();
    Schedules before = app.dbschedules().schedules();
    app.schedule().recordStudentOn2h(name, "recordOnSchedule");
  ...
  private void check(Schedules before, Schedules after) {
    ...
    ScheduleData scheduleAdd = new ScheduleData()
            ...
            .withSlots(Arrays.asList(new Slots()
                    ...
                    .withW(time.date())
                    .withSt(new ST().withS(time.Stime(period)).withE(time.Etime(period)))
                    .withC(Arrays.asList(new C().withId("recordOnSchedule").withType(3).withSubject("1")
